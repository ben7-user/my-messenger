...
def register():
    if request.method == 'POST':
        ...
        user = User(username=u, password_hash=hp(p))
        db.session.add(user)
        db.session.commit()
...
def login():
    if request.method == 'POST':
        ...
        if user and cp(p, user.password_hash):
            session['user_id'] = user.id
            session['username'] = user.username
            return redirect(url_for('chat'))
...
def api_chats():
    ...
    for cid in ids:
        ...
        mem = [{'id': m.user.id, 'username': m.user.username} for m in c.members]
        o = next((m for m in mem if m['id'] != me), None)
        t = c.name if c.is_group else (o['username'] if o else 'Chat')
        last = Message.query.filter_by(chat_id=cid).order_by(Message.timestamp.desc()).first()
        r.append({'id': cid, 'is_group': c.is_group, 'title': t, 'members': mem, 'last_message': {'text': last.text} if last else None})
    return jsonify(r)
...
def api_private():
    ...
    c = get_chat(session['user_id'], other)
    mem = [{'id': m.user.id, 'username': m.user.username} for m in c.members]
    o = next((m for m in mem if m['id'] != session['user_id']), None)
    return jsonify({'id': c.id, 'is_group': False, 'title': o['username'] if o else 'Chat', 'members': mem})
